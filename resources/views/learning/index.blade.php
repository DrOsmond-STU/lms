<x-layouts.app title="Pembelajaran Saya" workspace="participant">
    <h1 class="text-xl font-extrabold text-slate-800">Pembelajaran Saya</h1>
    <p class="mt-0.5 mb-6 text-sm text-slate-600">Riwayat dan progres pelatihan Anda.</p>
    <div class="grid gap-5 md:grid-cols-2">
        @forelse ($enrollments as $enrollment)
            <article class="card p-5">
                <div class="flex items-start justify-between gap-3">
                    ...
                    <span @class(['badge', 'bg-emerald-50 text-emerald-700' => $enrollment->status === 'passed', 'bg-amber-50 text-amber-700' => $enrollment->status === 'pending_approval', 'bg-rose-50 text-rose-700' => in_array($enrollment->status, ['failed', 'cancelled'], true), 'bg-brand-50 text-brand-700' => $enrollment->isActive()])>{{ $enrollment->statusLabel() }}</span>
                </div>
        ...
        @empty
            <div class="card p-8 text-center text-sm text-slate-500 md:col-span-2">Belum ada pelatihan. <a href="{{ route('catalog.participant') }}" class="font-bold text-brand-700 hover:underline">Pilih pelatihan</a>.</div>
        @endforelse
    </div>
</x-layouts.app>
