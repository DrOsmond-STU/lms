<x-layouts.app :title="$workspace === 'admin' ? 'Kelas & Jadwal' : 'Kelas Saya'" :workspace="$workspace">
    <h1 class="text-xl font-extrabold text-slate-800">{{ $workspace === 'admin' ? 'Kelas & Jadwal' : 'Kelas Saya' }}</h1>
    <p class="mt-0.5 mb-6 text-sm text-slate-600">{{ $workspace === 'admin' ? 'Semua kelas/batch. Kelas baru dibuat dari halaman program.' : 'Kelas yang Anda ampu: kelola materi, asesmen, dan penilaian.' }}</p>
    @if ($workspace === 'admin')
        ...
        <table class="data-table">
            ...
            <tbody>
                @forelse ($classes as $class)
                    <tr>
                        <td><a href="{{ route('classes.manage', $class) }}" class="font-bold text-brand-700 hover:underline">{{ $class->batch_name }}</a><span class="block text-xs text-slate-500">{{ $class->program->name }}</span></td>
