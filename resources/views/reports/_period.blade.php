<form method="GET" action="{{ $action }}" class="card mb-6 grid gap-3 p-4 sm:grid-cols-5" role="search">
    ...
    <div class="flex items-end gap-2">
        ...
        @can('report.export')<a href="{{ $exportRoute }}?{{ http_build_query(['dari' => $period['from']->toDateString(), 'sampai' => $period['to']->toDateString(), 'q' => $search ?? '']) }}" class="btn-primary w-auto">Ekspor CSV</a>@endcan
    </div>
</form>
